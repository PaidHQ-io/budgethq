/**
 * /api/oauth/meta/accounts?workspaceId=...
 *
 * Only relevant when a workspace's Meta token can see more than one ad account — callback.js
 * leaves `credential.accountId` as null in that case and redirects back with
 * ?meta_oauth=select_account instead of guessing. This route lets the SPA show a dropdown to
 * finish the connection. Mirrors api/oauth/linkedin/accounts.js exactly.
 *
 * GET  — list the ad accounts the workspace's stored Meta token can see.
 * POST Body: { accountId, accountName } — save which one to actually sync spend from. accountId
 *      is Meta's "act_..." form (see lib/metaOAuth.js's listAdAccounts) — stored and later passed
 *      straight through to the Insights endpoint as-is. accountName is stored purely for display
 *      (Settings' connections table) — listAdAccounts() already returns it, so this just persists
...
import { listAdAccounts } from "../../lib/metaOAuth.js";

async function getStoredCredential(workspaceId) {
  const rows = await sql`
    select credential from budgethq.connector_credentials
    where workspace_id = ${workspaceId} and provider = 'meta'
  `;
  if (!rows.length) {
    const err = new Error("This workspace hasn't connected Meta yet.");
    err.status = 400;
    throw err;
  }
