 ...
 *
 * PaidHQ used to own its own `workspaces`/`workspace_members` tables and this endpoint. Once
 * the plan became "sell PaidHQ, VaultHQ, ReportingHQ, AuditHQ under one PaidHQ account," that
...
export default withApi(async (req, res) => {
  return res.status(410).json({
    error: "This endpoint has moved. Workspace creation/listing now lives in paidhq-core's /api/workspaces — see PaidHQ's db/schema.sql for why.",
  });
});
