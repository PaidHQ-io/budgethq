    ...
    return(
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:40}}>
        <div style={{textAlign:"center",maxWidth:420}}>
          <div style={{fontSize:15*(T.fsScale||1),fontWeight:700,color:T.text,marginBottom:6,fontFamily:T.font}}>No data yet</div>
          <div style={{fontSize:13*(T.fsScale||1),color:T.textMuted,lineHeight:1.6,fontFamily:T.font}}>Connect a data source or upload a CSV/screenshot in Data Sources, or import a Dreamdata/PowerBI screenshot in Reporting Analyzer, then come back here to see coverage, gaps, and overlaps across everything that's been brought in.</div>
        </div>
      </div>
    );
  }
  ...
  return(
    <div style={{flex:1,overflow:"auto",padding:isMobilePad()?"16px":"24px 28px"}}>
      <div style={{maxWidth:1040,margin:"0 auto"}}>
        ...
        {!hasSpend?(
          <div style={{fontSize:12*(T.fsScale||1),color:T.textMuted,fontFamily:T.font,marginBottom:24}}>No spend data imported yet — connect a source or upload a CSV/screenshot in Data Sources.</div>
        ):(<>
        {/* Overview */}
        ...
        {overview.unparseableDates>0&&(
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",background:T.warningBg,border:`1px solid ${T.warningBorder}`,borderRadius:T.r8,marginBottom:14,fontSize:12*(T.fsScale||1),color:T.text,fontFamily:T.font}}>
        ...
        <PixelPanel T={T} style={{marginBottom:24,overflow:"hidden"}} contentStyle={{background:T.surface}}>
          <div style={{display:"grid",gridTemplateColumns:isMobilePad()?undefined:"1.6fr 1fr 1.4fr 0.7fr 0.9fr 0.7fr",gap:8,padding:"8px 14px",background:T.headerBg,borderBottom:`1px solid ${T.border}`,fontSize:10*(T.fsScale||1),fontWeight:700,letterSpacing:"0.05em",textTransform:"uppercase",color:T.textMuted}}>
          ...
          {bySource.map((s,i)=>(
            <div key={s.sourceKey} style={{display:"grid",gridTemplateColumns:isMobilePad()?undefined:"1.6fr 1fr 1.4fr 0.7fr 0.9fr 0.7fr",gap:8,padding:"10px 14px",borderTop:i>0?`1px solid ${T.border}`:"none",fontSize:12.5*(T.fsScale||1),color:T.text,alignItems:"center",fontFamily:T.font}}>
              <div style={{fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{sourceLabel(s.sourceKey)}</div>
              <div>
                <Pill color={sourceIsLive(s.sourceKey)?T.success:T.textSub} bg={sourceIsLive(s.sourceKey)?T.successBg:T.surfaceEl} border={sourceIsLive(s.sourceKey)?T.successBorder:T.border} style={{fontSize:10*(T.fsScale||1)}}>
              ...
              <PixelPanel key={p.platform} T={T} contentStyle={{background:T.surface,overflow:"hidden"}}>
                <div onClick={()=>toggle(p.platform)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:14,padding:"12px 16px",cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:9,minWidth:0}}>
                    ...
                    <span style={{fontSize:14*(T.fsScale||1),fontWeight:700,color:T.text,fontFamily:T.font,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.platform}</span>
                    {hasIssues&&(
                      <span style={{display:"flex",gap:5,flexShrink:0}}>
                        {p.gapDayCount>0&&<Pill color={T.danger} bg={T.dangerBg} border={T.dangerBorder} style={{fontSize:10*(T.fsScale||1)}}>{p.gapDayCount} gap day{p.gapDayCount===1?"":"s"}</Pill>}
                        {p.overlapRanges.length>0&&<Pill color={T.warning} bg={T.warningBg} border={T.warningBorder} style={{fontSize:10*(T.fsScale||1)}}>overlap</Pill>}
                      </span>
                    )}
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:16,flexShrink:0,fontSize:12*(T.fsScale||1),color:T.textSub,fontFamily:T.font}}>
                ...
                {open&&(
                  <div style={{padding:"0 16px 16px",borderTop:`1px solid ${T.border}`}}>
                    <div style={{display:"flex",gap:24,flexWrap:"wrap",padding:"12px 0"}}>
                      <div style={{fontSize:11*(T.fsScale||1),color:T.textMuted,fontFamily:T.font}}><strong style={{color:T.text}}>{p.rows.toLocaleString()}</strong> rows</div>
                      <div style={{fontSize:11*(T.fsScale||1),color:T.textMuted,fontFamily:T.font}}><strong style={{color:T.text}}>{p.campaigns.toLocaleString()}</strong> campaigns</div>
                      <div style={{fontSize:11*(T.fsScale||1),color:T.textMuted,fontFamily:T.font}}><strong style={{color:T.text}}>{p.sources.length.toLocaleString()}</strong> source{p.sources.length===1?"":"s"}</div>
                    </div>

                    <div style={{fontSize:11*(T.fsScale||1),fontWeight:700,color:T.textMuted,letterSpacing:"0.05em",textTransform:"uppercase",marginBottom:6}}>Sources feeding this platform</div>
                    <div style={{display:"flex",flexDirection:"column",gap:4,marginBottom:14}}>
                      {p.sources.map(s=>(
                        <div key={s.sourceKey} style={{display:"flex",justifyContent:"space-between",gap:10,fontSize:12*(T.fsScale||1),color:T.text,fontFamily:T.font}}>
                    ...
                    {p.gapRanges.length>0&&(
                      <>
                        <div style={{fontSize:11*(T.fsScale||1),fontWeight:700,color:T.danger,letterSpacing:"0.05em",textTransform:"uppercase",marginBottom:6}}>Missing days ({p.gapDayCount})</div>
                        <div style={{fontSize:11*(T.fsScale||1),color:T.textMuted,marginBottom:6,fontFamily:T.font}}>No data from any source on these calendar days, within this platform's own {fmtRange(p.start,p.end)} span.</div>
                        <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:14}}>
                          {p.gapRanges.map((r,i)=>(
                            <Pill key={i} color={T.danger} bg={T.dangerBg} border={T.dangerBorder} style={{fontSize:10.5*(T.fsScale||1)}}>
                    ...
                    {p.overlapRanges.length>0&&(
                      <>
                        <div style={{fontSize:11*(T.fsScale||1),fontWeight:700,color:T.warning,letterSpacing:"0.05em",textTransform:"uppercase",marginBottom:6}}>Overlapping coverage</div>
                        <div style={{fontSize:11*(T.fsScale||1),color:T.textMuted,marginBottom:6,fontFamily:T.font}}>These sources both claim data for the same days — only one value is stored per campaign/day (the most recently synced or imported wins), so this doesn't necessarily mean the numbers disagreed, just that it was possible.</div>
                        <div style={{display:"flex",flexDirection:"column",gap:5}}>
                          {p.overlapRanges.map((o,i)=>(
                            <div key={i} style={{fontSize:12*(T.fsScale||1),color:T.text,fontFamily:T.font}}>
        ...
        {reportingLoading?(
          <div style={{fontSize:12*(T.fsScale||1),color:T.textMuted,fontFamily:T.font,marginBottom:24}}>Loading…</div>
        ):reportingFactsError?(
          <div style={{padding:"9px 12px",background:T.dangerBg,border:`1px solid ${T.dangerBorder}`,borderRadius:T.r8,fontSize:12*(T.fsScale||1),color:T.danger,fontFamily:T.font,marginBottom:24}}>{reportingFactsError}</div>
        ):!hasReporting?(
          <div style={{fontSize:12*(T.fsScale||1),color:T.textMuted,fontFamily:T.font,marginBottom:24}}>Nothing imported yet — upload a Dreamdata/PowerBI screenshot in Reporting Analyzer.</div>
        ):(<>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:18}}>
          ...
          <>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:24}}>
              {tagCompleteness.map(t=>(
                <div key={t.dimension} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:T.surface,border:`1px solid ${t.missing>0?T.warningBorder:T.border}`,borderRadius:T.r8,fontSize:12*(T.fsScale||1),fontFamily:T.font}}>
        ...
        <SectionLabel T={T} style={{marginBottom:8}}>By source</SectionLabel>
        <PixelPanel T={T} style={{marginBottom:24,overflow:"hidden"}} contentStyle={{background:T.surface}}>
          <div style={{display:"grid",gridTemplateColumns:isMobilePad()?undefined:"1.8fr 1.6fr 0.7fr 0.9fr",gap:8,padding:"8px 14px",background:T.headerBg,borderBottom:`1px solid ${T.border}`,fontSize:10*(T.fsScale||1),fontWeight:700,letterSpacing:"0.05em",textTransform:"uppercase",color:T.textMuted}}>
            <div>Source</div><div>Date range</div><div style={{textAlign:"right"}}>Rows</div><div style={{textAlign:"right"}}>Campaigns</div>
          </div>
          {rBySource.map((s,i)=>(
            <div key={s.sourceKey} style={{display:"grid",gridTemplateColumns:isMobilePad()?undefined:"1.8fr 1.6fr 0.7fr 0.9fr",gap:8,padding:"10px 14px",borderTop:i>0?`1px solid ${T.border}`:"none",fontSize:12.5*(T.fsScale||1),color:T.text,alignItems:"center",fontFamily:T.font}}>
            ...
            return(
              <PixelPanel key={p.periodType} T={T} contentStyle={{background:T.surface,overflow:"hidden"}}>
                <div onClick={()=>toggleReporting(p.periodType)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:14,padding:"12px 16px",cursor:"pointer"}}>
                  <div style={{display:"flex",alignItems:"center",gap:9,minWidth:0}}>
                    <span style={{fontSize:14*(T.fsScale||1),fontWeight:700,color:T.text,fontFamily:T.font}}>{PERIOD_TYPE_LABELS[p.periodType]||p.periodType}</span>
                    {hasIssues&&(
                      <Pill color={T.danger} bg={T.dangerBg} border={T.dangerBorder} style={{fontSize:10*(T.fsScale||1)}}>{p.gapPeriodCount} missing period{p.gapPeriodCount===1?"":"s"}</Pill>
                    )}
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:16,flexShrink:0,fontSize:12*(T.fsScale||1),color:T.textSub,fontFamily:T.font}}>
                    <span>{fmtRange(p.start,p.end)}</span>
                ...
                {open&&(
                  <div style={{padding:"0 16px 16px",borderTop:`1px solid ${T.border}`}}>
                    ...
                    {p.gapRanges.length>0?(
                      <>
                        <div style={{fontSize:11*(T.fsScale||1),fontWeight:700,color:T.danger,letterSpacing:"0.05em",textTransform:"uppercase",marginBottom:6}}>Missing periods ({p.gapPeriodCount})</div>
                        <div style={{fontSize:11*(T.fsScale||1),color:T.textMuted,marginBottom:6,fontFamily:T.font}}>No {(PERIOD_TYPE_LABELS[p.periodType]||p.periodType).toLowerCase()} row for these periods, within this grain's own {fmtRange(p.start,p.end)} span.</div>
                      ...
                      <div style={{fontSize:11*(T.fsScale||1),color:T.textMuted,fontFamily:T.font}}>No gaps within this grain's own span.</div>
                    )}
                  </div>
                )}
              </PixelPanel>
