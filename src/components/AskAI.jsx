...
export default function AskAI({T,session,mergedNormRows,tags,tagDims,budgetDims,budgets,budgetRowMeta,defaultForecastModel,hasData,askChats,setAskChats,askProjects,setAskProjects,activeAskChatId,setActiveAskChatId,sidebarEl,initialQuestion,onConsumeInitialQuestion,onSaveAsView,combineGoogleChannels=false}){
  ...
  const chatMenu=chatMenuOpenId&&chatMenuAnchorRect&&createPortal(
    <>
      ...
      <div style={{position:"fixed",top:chatMenuAnchorRect.bottom+4,left:Math.max(8,chatMenuAnchorRect.right-224),zIndex:1000,width:224,maxHeight:360,overflow:"auto",background:T.surface,border:`1px solid ${T.border}`,borderRadius:T.r8,boxShadow:T.shadowMd,padding:6,display:"flex",flexDirection:"column",gap:2}}>
        {(()=>{
