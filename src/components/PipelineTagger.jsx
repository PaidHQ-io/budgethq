...
// Pipeline Tagger (2026-08-01, per Mo — "let's bring it all in and tag it the same way we're
// tagging our campaigns... analyze it in isolation"). Mirrors Campaign Tagger's UX — filter,
// bulk-select, apply-a-tag-dimension-to-selected, undo — but operates on core.reporting_facts
// (Dreamdata/PowerBI/eventually HubSpot/Salesforce import data) instead of spend_rows, using the
// exact same tag_dims vocabulary (Product/Region/Funnel/Pillar/etc., whatever this workspace has
// configured) so a segment means the same thing whether you're looking at ad spend or pipeline.
//
...
// time) stays exactly as imported. This tab's only job is getting every reporting_facts row tagged
// with the same dimensions Campaign Tagger uses, so a later analysis view can slice pipeline
...
export default function PipelineTagger({ T, session, workspace, campaignTags, tagDims, canEdit }) {
  ...
  const filtered = useMemo(() => {
    ...
    return groups.filter((g) => {
      ...
      // Selected groups are exempt from the tagged/untagged status filter (2026-08-01 — same fix
      // as Campaign Tagger's own filtered memo): applying a dimension to a selected "needs review"
      // group moves it into "tagged," which would otherwise yank it off screen — and out of the
      // selection — mid multi-dimension tagging session, right when you still need it there to
      // apply the next dimension.
  ...
  // Cross-match against Campaign Tagger's own tags (2026-08-01, per Mo — same tagging effort
  // shouldn't have to happen twice for a campaign you've already tagged for ad spend). Exact,
  // case-insensitive match only, against either half of a campaignKey (group name or ad/campaign
  // name) — deliberately NOT fuzzy/substring, since a false-positive tag applied to the wrong
  // pipeline campaign is worse than making the user tag it manually. One click applies every
  // dimension the matched ad campaign already has that this pipeline campaign is still missing —
  // never overwrites a dimension this campaign already has a value for.
  ...
  return (
    <div style={{ padding: 28, maxWidth: 1300, margin: "0 auto", fontFamily: T.font, overflow: "auto", height: "100%", boxSizing: "border-box" }}>
      ...
      <div style={{ fontSize: 16 * (T.fsScale || 1), fontWeight: 700, color: T.text, marginBottom: 6 }}>Pipeline Tagger</div>
      <div style={{ fontSize: 13 * (T.fsScale || 1), color: T.textSub, lineHeight: 1.6, marginBottom: 20 }}>
        ...
        already imported in Performance Intelligence.
      </div>
      ...
      {groups.length === 0 && !loadError ? (
        <PixelPanel T={T} contentStyle={{ padding: 40, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 13 * (T.fsScale || 1), color: T.textSub }}>Import a screenshot or CSV in Performance Intelligence first.</div>
        </PixelPanel>
      ) : (
        <>
