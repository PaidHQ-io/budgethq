...
// buildPdfDoc does the actual drawing and returns the jsPDF doc object undownloaded — split out
// (2026-08-19, per Mo's "move create_asset over" Ask AI request) so Ask AI's create_asset tool can
// get the generated bytes back as a Blob to upload as a Vault attachment, instead of the browser
// triggering a save-to-disk dialog the way the existing export button correctly still does.
function buildPdfDoc(entry) {
...
export function exportEntryAsPdf(entry) {
  buildPdfDoc(entry).save(`${(entry.title || "vault-entry").replace(/[\\/:*?"<>|]/g, "")}.pdf`);
}

// Blob variant for create_asset (2026-08-19) — same doc, returned as a Blob instead of downloaded.
export function exportEntryAsPdfBlob(entry) {
  return buildPdfDoc(entry).output("blob");
}

// ─── PPTX (pptxgenjs) — one slide per ## section, matching VaultHQ's own shape ──────────────────
// buildPptxObj mirrors buildPdfDoc's split — returns the built pptx object before it's written out,
// so both the download button and create_asset's Blob path share the exact same slide-building logic.
async function buildPptxObj(entry) {
...
export async function exportEntryAsPptx(entry) {
  const pptx = await buildPptxObj(entry);
  await pptx.writeFile({ fileName: `${(entry.title || "vault-entry").replace(/[\\/:*?"<>|]/g, "")}.pptx` });
}

// Blob variant for create_asset (2026-08-19) — pptxgenjs's write({outputType:"blob"}) resolves a
// Blob directly rather than triggering a download, same relationship as exportEntryAsPdfBlob above.
export async function exportEntryAsPptxBlob(entry) {
  const pptx = await buildPptxObj(entry);
...
function buildXlsxWorkbook(entry) {
  const blocks = parseMarkdownBlocks(entry.content);
...
export function exportEntryAsXlsx(entry) {
  const out = XLSX.write(buildXlsxWorkbook(entry), { bookType: "xlsx", type: "array" });
  triggerDownload(new Blob([out], { type: XLSX_MIME }), `${(entry.title || "vault-entry").replace(/[\\/:*?"<>|]/g, "")}.xlsx`);
}

// Blob variant for create_asset (2026-08-19) — same workbook, returned as a Blob instead of downloaded.
export function exportEntryAsXlsxBlob(entry) {
  const out = XLSX.write(buildXlsxWorkbook(entry), { bookType: "xlsx", type: "array" });
