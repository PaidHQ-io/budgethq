...
// Inverse of localISODate, for display — takes a date-only value (as stored in
...
//
// FIX (2026-08-05, take 2): an earlier version of this helper hand-parsed `s.split("-")`,
// assuming every caller's value is a plain "YYYY-MM-DD" string. It isn't — r.date (and therefore
// importDateRangeByProvider's start/end) comes straight from each connector/import path and can
// be a full ISO timestamp like "2026-08-04T00:00:00.000Z" (see mergeRows's doc comment above,
// which already documents this exact inconsistency). Splitting a timestamp string on "-" pulls
// "04T00:00:00.000Z" as the day component, Number()s to NaN, and renders "Invalid Date" — caught
// live the same day this shipped, via Google Ads' sidebar showing that literal text. parseSpendDate
// already handles every format this app produces (plain date, timestamp, slash-date, month-year,
// etc.) and constructs its Date in local time the same way localISODate expects, so delegate to it
// instead of re-solving date parsing here.
export const fmtCalendarDate=(s,opts)=>{
  const d=parseSpendDate(s);
